...
| 48-49 | 6.1 — Tests isolation / integration | ✅ done | cycle 48-49 landed — 43+ integration теста (b2ca9d0: status-workflow 14 + auto-receive-finished-goods 11 + helper mockPrisma.ts; 6ec0d4a Part 3: env 7 + clone-items 12); vitest full suite 337+/337 |
...
| 2026-06-22 (cycle 57) | **Cycle 57 DONE (B.7 UserActivity UI — last business block!)** — Полный end-to-end audit trail: NEW `src/lib/__tests__/activity-log.test.ts` (10 vitest tests, best-effort + JSON serialization + error swallowing) + 20 API routes wired (12 POST creates + 5 PATCH status transitions + 2 convert handlers + 1 login — proposals, contracts, production-orders, supplier-orders, incoming-invoices, materials, products, categories, product-modules, persons, organizations) + `<ActivityLog entity entityId />` integrated в 2 viewer pages (`proposals/[id]/page.tsx` + `contracts/[id]/page.tsx`) под `ContractPreview`/`ProposalPreview` панелями. ADR-007 (6 architectural decisions: AWAIT pattern, entity/action namespace, minimal details payload, snake_case entity strings, userName fallback chain, UI load pattern). Gates: tsc 0 / vitest green (294+ tests) / eslint 0. |
| 2026-06-22 (cycle **P0+P1 quick wins**) | **Cycle P0+P1 quick wins DONE**: scope=user-selected ("доделай по чеклистам" → "Что для проекта"). Bases: **P0.2** Sync `BUSINESS-LOGIC.md` v3.0→3.1 (stages 5/6/9/10/12 + §6.2 all 7 counters + §7 all 6 FK relations flipped 🔴→✅ с code refs); **P1.3** ESLint: базово 0/0 (cleanup уже сделано ранее — scope shrink); **P1.4** Build: SUCCESS verified (Static + Dynamic/Proxy routing); **P1.PERF-AUDIT** `PERFORMANCE_CHECKLIST.md` header — pointer to ✅ `PERFORMANCE_OPTIMIZATION_CHECKLIST.md` (8/8 steps реализованы, FCP 300→124ms, Page Weight 3.28→2.80 MB). **`CURRENT-CHECKLIST.md` + `MASTER-CHECKLIST.md` synced** — P0.2, P1.3, P1.4, P1.PERF-AUDIT → ✅ DONE. Gates: tsc 0 / vitest 380/380 / eslint 0/0 / build OK. |
| 2026-06-22 (cycle **P2.1 / Cycle 54 Organization.type discriminator**) | **Cycle P2.1 DONE**: user-scoped ("Стартуй новую сессию с P2.1 — Cycle 54"). Schema + Zod DU были уже написаны в prior commit, но 1 PROD BUG + dead code остались: **(1)** UI form `OrganizationForm` НЕ отправлял `type` → `POST /api/organizations` через Zod DU падал с 400 → UI не работает. **Fix:** server-side `pickValidType()` helper в POST route — coerces payload к `OrganizationType` literal с fallback `'legal'` если отсутствует/пустое/невалидное. **(2)** `applyTypeAwareValidation` exported, но никем не вызывался → PUT не делал DB-type-aware ИНН/КПП refinement. **Fix:** wire-in в PUT — `findUnique` с select `id, type` → VALID_TYPES guard (500 если unknown) → `validateBody(UpdateOrg)` → `applyTypeAwareValidation(parsed, existing.type, mockCtx)` → 400 если issues. **(3)** Organization validation tests MISSING из `validations-reference.test.ts` (только Certificate/RppEntry/Worker/WorkType/WorkCenter/StorageItem). **Fix:** NEW `src/lib/__tests__/validations-organization.test.ts` — 27 тестов (3 DU happy × 3 variants + discriminator reject + INN mismatch × 2 + КПП strip + 6 Update shape + 7 Update superRefine + 8 applyTypeAwareValidation mock-ctx + 8 pickValidType). Migration `20260622000000_add_organization_type_discriminator` уже существует (CHECK constraint закомментирован — soft-enforcement). Code-reviewer SHIP-READY (3 rounds: original → empty-string bug → type filter finalize). Gates: tsc 0 / vitest 407/407 (+27) / eslint 0/0 / build OK. |
