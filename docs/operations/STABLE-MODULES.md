...
| `src/lib/number-protection.ts` | 55 | ✅ 0 | ✅ 12/12 (number-protection.test.ts) | pending ADR | ✅ manual | **CANDIDATE** — B.4 frozen-number immutability helper (`assertNumberImmutable` + `NumberLockedError`). Wired в 5 PATCH routes: proposals / contracts / production-orders / supplier-orders / incoming-invoices. Status config в `@/lib/constants/frozen-statuses` (5 entity × per-doc frozen lists). Pending Tier A promotion. |
| `src/lib/activity-log.ts` | 57 | ✅ 0 | ✅ 7/7 (activity-log.test.ts) | pending ADR | ✅ manual | **CANDIDATE** — B.7 audit + timeline helper (`recordActivity`, best-effort, never breaks primary request). UI: `<ActivityLog>` (timeline w/ pagination). API: `GET /api/activity-log` + 18+ producers. Pending Tier A promotion. |
...
| 2026-06-23 | Tier C expansion (cycles 55-57): `number-protection.ts` (B.4 frozen-number immutability — 12 tests) + `activity-log.ts` (B.7 audit + timeline — 7 tests). Cycle 56 = schema-only (OrderClosing.order FK SetNull, no new helper). Tier A/B/C unchanged. Pending ADR capture. | 55-57 |
