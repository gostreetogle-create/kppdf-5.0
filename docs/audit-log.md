...
<a id="cycle-adr-rev2"></a>
## Cycle adr-rev2 (docs) — 2026-06-20 — ADR-005-rev2 ProposalEditor React memoization

**Context:** User requested ADR documenting pdfData snapshot semantics (cycle-45 polish) + 4 deferred follow-ups (F1 dep-array bloat, F2 snapshot semantics IS this ADR, F3 cartItems snapshot consistency, F4 finance exposure). Cycles 44-47 (B.3 Block 3.1 + Block 4.1) wrote substantial React.memo + useMemo + Context bag architecture but lacked an architectural decision record.

### What shipped

- **docs/decisions/ADR-005-rev2-proposal-editor-react-memoization.md** (new, ~340 lines, status="proposed")
  - Section Context: cycles 44/45/47 chronological memoization evolution
  - Section Decision: client-side caching via React.memo + useMemo + Context bag (single Provider)
  - Section Consequences: Positive (compiler no bailout, dep-array 7) + Negative (cognitive load, allocation overhead)
  - Section Re-render matrix: 11 Context consumers re-render together (rev3 empirical truth)
  - Section Considered alternatives (rev4 restructure): Rejected (Zustand, blanket React.memo, useRef+shallow-equal) vs Deferred (Split Context F5-A + F5-C + F1 + F3 + F4)
  - Section F5 (new in rev3): Single-Context cascade - Context split as HIGH-priority deferral to cycle-48-extension; cost-benefit F5-A (Context split) vs F5-B (value stabilization) vs F5-C (remove dead memo); cycle-47 ResizableEditorLayout integration addendum (rev4) = single-root 3-Provider wrap NOT per-Panel Providers
  - Section Tier classification: C (candidate for promotion to B after cycle-48-49 test coverage)
- rev3 ack: React.memo(ProductSelector) is dead code (zero props). Architectural correction per Agent A round-1 review.
- rev4 cleanup: removed broken ADR-006 cross-ref; restructured Section Rejected alternatives around Rejected/Deferred semantic split; added F5-A integration addendum for cycle-47 ResizableEditorLayout; corrected matrix caption 7-of-11.
- rev5 self-contradiction fix: Cycle 47 prescriptive "1:1 mapping panel-provider" contradicted F5-A "NOT 1:1" stance; replaced with neutral deferral pointing to F5-A as single authoritative boundary-decision location; fixed "UX-orzh" gibberish typo.

### Code-reviewer verdict history

- Round-1 (rev2 to rev3): NO - critical architectural misunderstanding (memo is dead code).
- Round-2 (rev3 to rev4): NO - 3 BLOCKERS (broken ADR-006 cross-ref, contradictory Section Rejected alternatives, F5-A missing cycle-47 integration).
- Round-3 (rev4 to rev5): NO - 1 new contradiction introduced in Cycle 47 + 1 typo.
- Round-4 (rev5 to committed): YES - SHIP-READY.
...
- Tier integrity: A (lib/jwt.ts) / B (lib/pdf/index.ts) UNTOUCHED.

**Rule 6 compliance:** docs-only commit - single file staged (the unrelated dirty working tree from prior unfinished cycles is preserved unstaged per Rule 5 minimal-change discipline).
<a id="cycle-bootstrap-escape"></a>
## Cycle bootstrap-escape (hotfix) — 2026-06-20 — /api/seed chicken-and-egg fix

**Context:** User reported login admin / admin123 returns 401 in dev. Diagnostic chain:
- POST /api/auth/login → prisma.user.findUnique returns null (no admin User row in DB)
- POST /api/seed bootstrap route required requireRole(['admin']) BEFORE creating admin
- Chicken-and-egg deadlock: seed needs admin to create admin = impossible bootstrap
- Self-blocking: developer can never bootstrap a fresh dev DB

**Fix (this commit):** src/app/api/seed/route.ts adds `adminCount === 0` bootstrap escape:
- adminCount = await prisma.user.count({ where: { role: 'admin' } })
- if (adminCount === 0) → bootstrap path (log warning, no auth required)
- else → requireAuth + requireRole(['admin']) as before

Self-disables after first seed run. Safe in dev (bootstrapping) AND in prod (zero-day init).

**Companion infra (NOT in commit scope, applied live to dev DB during verification):**
- `npx prisma migrate deploy` applied 4 pending migrations including cycle-54 B.2
  Client.type legal-entity fields (column type discriminator default 'individual').
  Bootstrap POST /api/seed was returning HTTP 500 P1013 (column does not exist)
  until migration applied.
- DELETE FROM "RateLimitEntry" cleared cycle-57 B.7 rate-limit lock (1 row)
  so previously-blocked logins succeed. Login route was returning HTTP 429.

**Verification:** POST /api/auth/login admin / admin123 → HTTP 200 + Set-Cookie
accessToken + refreshToken + JSON {success:true, data:{user:{username:'admin', role:'admin'}}}.
User can now navigate from /login to /dashboard normally.

**Known pre-existing (NOT introduced, NOT in commit scope):** /api/seed is NOT
idempotent for non-User entities (orgs/products/clients/warehouses/etc. use
.create() without upsert / deleteMany). Re-running after bootstrap will duplicate
non-User rows. Future cycle should add idempotent guards.

**Code-reviewer verdict:** SHIP-READY YES (round-1 after thinker Option A
bootstrap escape acceptable per docs/CONTRIBUTING.md Rule 1 minimal-change
discipline).

**Tier integrity:** A (`@/lib/jwt.ts`) / B (`@/lib/pdf/index.ts`) UNTOUCHED.

Rule 6 compliance: hotfix commit = src/app/api/seed/route.ts only (1 file).
Unrelated dirty working tree (38+ files from prior unfinished cycles 30-50)
preserved unstaged per Rule 5.
<a id="cycle-design-tokens-v3"></a>

### Cycle design-tokens-v3: «Neutral Slate + Deep Violet» — 20.06.2026

**Контекст**: пользователь сообщил о трёх проблемах визуального качества:
1. Тёмная тема имела паразитный зеленоватый отлив (фон `#0d1a11`, primary `#4ade80`, foreground `#d4e8d0` — toxic-green UI).
2. В светлой теме левое меню оставалось тёмно-коричневым (`--sidebar: #1a1510`) — баг цветовой схемы.
3. Светлая тема содержала «бардак»: кремовый фон, бronze primary, банальные голубые акценты — выглядела устаревшей.

**Задача**: академический рефакторинг ТОЛЬКО цветовых токенов (без изменения layout/typography/lógica) с целью сделать обе темы стильными и профессиональными.

**Решение**: дизайн-система v3.0 «Neutral Slate + Deep Violet». Полная переработка CSS-переменных.

#### DARK theme — mix 3 референсов
- **Kaspersky** (обязательный): синевато-сланцевая база `#0B0E14` без намёка на зелень, crisp 1px границы `#2A2E38`.
- **Linear**: 5-уровневая tint-elevation лестница (bg < sidebar < muted < card < popover), различение через subtractive LIGHTENING.
- **Vercel**: ультра-глубокий черный base + pure-black shadows (alpha до 0.60) для глубины.

#### LIGHT theme — accent Deep Violet
- Выбран **Violet-700 #7C3AED** (light) / **Violet-400 #A78BFA** (dark).
- Обоснование: WCAG AA 5.4:1 на белом; brand-coherence через одну hue-семью; дифференциация от banal blue SaaS (Stripe/Apple indigo); совместимость с Linear family.
- Баг light-theme sidebar **исправлен**: `#FFFFFF` вместо `#1a1510`.

#### Glassmorphism (full)
- Новые токены `--glass-bg/border/highlight` + utility class `.glass-surface`.
- Topbar: `bg-/75 backdrop-blur-2xl` + 1px accent gradient line на верхнем ребре.
- Sidebar: `bg-/95 backdrop-blur-2xl` (full glass, видно когда контент проходит за меню).
- User dropdown: `bg-/85 backdrop-blur-xl` + rounded-xl + shadow-lg.

#### WCAG AA verification
Все ключевые проверены:
- Light foreground on bg: 17.4:1 (AAA) | Primary on bg: 5.4:1 (AA) | Sidebar-fg on sidebar: 16.6:1 (AAA)
- Dark foreground on bg: 14.1:1 (AAA) | Primary on bg: 8.2:1 (AAA) | Sidebar-fg on sidebar: 11.0:1 (AAA)
- **Light `--info` fix**: было `#06B6D4` (cyan-500) = 2.45:1 FAIL → стало `#0E7490` (cyan-700) = 5.4:1 PASS. Aligns with `--status-info-text`.

#### Anti-patterns устранены
- ❌ Кремовый фон (`#fefaf2`) → ✅ airy cool-white (`#FBFBFD`)
- ❌ Тёплые brown shadows (`rgb(139 115 85)`) → ✅ neutral cool-slate (`rgb(15 17 21)`)
- ❌ Green-tinted dark theme → ✅ slate + violet, без green-channel bias
- ❌ Dark sidebar в light theme → ✅ LIGHT sidebar (`#FFFFFF`)
- ❌ Banal blue accent → ✅ Deep Violet (distinctive)

#### Файлы изменены
- `src/app/globals.css` — full rewrite (378+/153-)
- `src/components/layout/sidebar.tsx` — glass surface + comment fix
- `src/components/layout/topbar.tsx` — glass + accent gradient + dropdown glass
- `UI_KIT.md` — token table rewrite + new §1.6 Дизайн-философия + history v3.0 row

#### Gates
- `tsc --noEmit`: **0 errors** ✅
- `eslint src`: **0 errors** ✅
- `vitest run`: **143/143 passed** ✅
- WCAG AA: **verified** via WebAIM contrast rules
- Code-reviewer round-2 verdict: **SHIP-READY ✅**

#### Polish note (non-blocking)
- `--glass-bg/border/highlight` tokens + `.glass-surface` utility defined but currently zero consumers reference them (sidebar/topbar/dropdown вместо этого используют arbitrary Tailwind opacity modifiers). Either keep as forward-looking seam OR YAGNI-cleanup. Not blocking.

#### Refactor Approach
- Single thematic commit per docs/CONTRIBUTING.md Rule 6.
- Code-reviewer verdict SHIP-READY получено без доработок по critical concerns.
- No new tests added (тестирование дизайн-системы — visual smoke, в scope браузерной верификации, не vitest unit tests).
- UI_KIT.md обновлён как единый источник истины для дизайн-токенов + дизайн-философии.

**Готово к использованию**: переключение темы через `data-theme="dark"` атрибут на `<html>` элементе — работает без изменений благодаря cascade через `var(--X)` consumers.


---

## Cycle 41 (2026-06-20) — docs cleanup (Phase 0-4)

### Theme
Полная реструктуризация документации проекта. Cycle v3.1.2 (после v3.1.1 SCRAM-fix), реализует директиву «Генеральная уборка docs/».

### Stats
- **Removed**: 31 мусор-файлов (6 *.md-pre-toc leftovers, 8 audit-log.md.bak/c*bak, 1 questions.md.bak, 10 agent-queue.json.bak*, 4 *.txt scratch, 1 .tmp-glass-cycle-audit-entry.md, 1 untracked ai-code-review.md)
- **Moved+Renamed**: 22 файла (MOVE+RENAME в docs/{process,domain,design,operations,analysis,spec,checklists})
- **Archived**: 8 архивных (1 doubled-archive, 4 plans, 1 critical analysis, 1 project_ready, 1 spec-dorabotka)
- **New docs**: 6 навигационных (docs/README.md, docs/GLOSSARY.md, docs/_archive/README.md, docs/_cleanup/{AUDIT_REPORT,PROPOSED_STRUCTURE,CLEANUP_FINAL_REPORT}.md)
- **Cross-refs fixed**: docs/CONTRIBUTING.md (8 paths), README.md (3 paths)

### Result
- Root .md: 33 → 3 (AGENTS.md, CLAUDE.md, README.md)
- docs total: 39 файлов в docs/
- Zero broken cross-refs validated
- Zero точных дубликатов remaining

### Decisions
- Не merge для SPEC.md + SPEC-ДОРАБОТКА.md (последняя имеет «Статус: Завершено», историческая)
- AGENTS.md, CLAUDE.md остались в корне (tooling читает: agent-cli.js check + Anthropic convention)
- CONTRIBUTING.md — только cross-refs (содержание = Конституция, не трогаем)
- Содержимое docs/decision/* не тронуто (ADR chain)
